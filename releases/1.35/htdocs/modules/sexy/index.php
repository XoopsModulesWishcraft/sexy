<?php
	...
	case "profile":
		...
		default:	
			...
			$url = XOOPS_URL."/".$GLOBALS['xoopsModuleConfig']['baseofurl']."/".xoops_sef($profile->getVar('sexuality'))."/".xoops_sef($profile->getVar('alias'))."/profile,".$pid.$GLOBALS['xoopsModuleConfig']['endofurl'];
			...
			$GLOBALS['xoopsTpl']->assign('xoops_pagetitle', implode(' - ', $profile_profile->getVar('type')) . $profile->getVar('alias') . ' - ' . ucfirst($profile_profile->getVar('sex')) . ' Actor ( '. $profile->getVar('sexuality'). ' )');
	...
	default:
		...
		$url = XOOPS_URL."/".$GLOBALS['xoopsModuleConfig']['baseofurl']."/index,".$op.','.$fct.','.$start.','.$limit.$GLOBALS['xoopsModuleConfig']['endofurl'];
		if (!strpos($url, $_SERVER['REQUEST_URI'])&&$GLOBALS['xoopsModuleConfig']['htaccess']) {
			header( "HTTP/1.1 301 Moved Permanently" ); 
			header( "Location: ".$url);
			exit;
