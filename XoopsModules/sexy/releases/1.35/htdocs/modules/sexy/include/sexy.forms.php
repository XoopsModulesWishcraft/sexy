<?php
	...
	function sexyPhysiqueForm($id)
	{
		...
		if ($id>0)	{
			...
			if (@$pprofile->getVar($GLOBALS['xoopsModuleConfig']['profile_sextype_field'])==$GLOBALS['xoopsModuleConfig']['profile_male_value'])
				$male=true;
			else
				$male=false;
		...
		$formobj['sex'] = new XoopsFormSelectPhysiqueSex(_SXY_MF_PHYSIQUE_SEX, $GLOBALS['xoopsModuleConfig']['profile_sextype_field'], ($physique->getVar($GLOBALS['xoopsModuleConfig']['profile_sextype_field'])));
	...
	function sexyProfileForm($pid)
	{
		...
		if (is_object($GLOBALS['xoopsUser'])) {
			...
			if (is_object($hosts[0])) {
				$formobj['host_id'] = new XoopsFormHidden('host_id', $hosts[0]->getVar('host_id'));
				$formobj['webcams_username'] = new XoopsFormHidden('webcams_username', $GLOBALS['xoopsUser']->getVar('uname'));
			}
		}
		$formobj['submit'] = new XoopsFormButton('', 'submit', _SUBMIT, 'submit');
	...
	function sexyEditUploadPicture($pid=0, $id=0)
	{
		...
		$rotate_radio->addOption( 'rot90' , "<img src='".XOOPS_URL."/modules/sexy/images/icon_rotate90.gif' alt='"._SXY_RADIO_ROTATE90."' title='"._SXY_RADIO_ROTATE90."' /> &nbsp; " ) ;
		$rotate_radio->addOption( 'rot180' , "<img src='".XOOPS_URL."/modules/sexy/images/icon_rotate180.gif' alt='"._SXY_RADIO_ROTATE180."' title='"._SXY_RADIO_ROTATE180."' /> &nbsp; " ) ;
		$rotate_radio->addOption( 'rot270' , "<img src='".XOOPS_URL."/modules/sexy/images/icon_rotate270.gif' alt='"._SXY_RADIO_ROTATE270."' title='"._SXY_RADIO_ROTATE270."' /> &nbsp; " ) ;
